import requests
import time
import csv
import pandas as pd
import json
import concurrent.futures
from pathlib import Path
from requests.adapters import HTTPAdapter
from requests.packages.urllib3.util.retry import Retry
from tqdm import tqdm
from tenacity import retry, wait_exponential, stop_after_attempt

PROJECT_ROOT = Path(__file__).resolve().parents[2]
DATA_DIR = PROJECT_ROOT / "data"
output_path = DATA_DIR / "menu_data.csv"
input_path = DATA_DIR / "delivery_info_clean.csv"
delivery_df = pd.read_csv(input_path)
id_list = delivery_df[["id", "restaurant_id"]]


def init_session():
    """Create a reusable session with retry support."""
    session = requests.Session()
    retries = Retry(total=3, backoff_factor=0.5, status_forcelist=[500, 502, 503, 504])
    session.mount("https://", HTTPAdapter(max_retries=retries))
    return session


@retry(wait=wait_exponential(multiplier=1, min=2, max=10), stop=stop_after_attempt(5))
def request_dish_detail(session, restaurant_id):
    """Send an API request with retry logic to fetch dish details."""
    response = session.get(
        "https://gappapi.deliverynow.vn/api/dish/get_delivery_dishes",
        params={"id_type": 2, "request_id": restaurant_id},
        headers={
            "X-Foody-Access-Token": "",
            "X-Foody-Api-Version": "1",
            "X-Foody-App-Type": "1004",
            "X-Foody-Client-Id": "",
            "X-Foody-Client-Language": "vi",
            "X-Foody-Client-Type": "1",
            "X-Foody-Client-Version": "3.0.0",
        },
        timeout=(3, 10),
    )
    response.raise_for_status()
    return response


def get_dish_details(session, request_id, restaurant_id):
    """Fetch dish information for a restaurant_id."""
    dishes_list = []
    response = request_dish_detail(session, request_id)
    data = response.json()

    # Extract the dish name and price from each menu entry.
    menu_infos = data.get("reply", {}).get("menu_infos", [])
    for menu in menu_infos:
        dishes = menu.get("dishes", [])

        for dish in dishes:
            name = dish.get("name", "")
            price = dish.get("price", {}).get("value", "")
            if name and price:
                dishes_list.append({"name": name, "price": int(price)})

    return {"restaurant_id": restaurant_id, "menu": dishes_list}


def write_to_csv(output_csv, data):
    """Write results to CSV with the menu stored as JSON strings."""
    records = [
        {
            "restaurant_id": item["restaurant_id"],
            "menu": json.dumps(item["menu"], ensure_ascii=False),
        }
        for item in data
    ]
    df = pd.DataFrame(records)
    df.to_csv(
        output_csv, mode="a", header=False, index=False, quoting=csv.QUOTE_NONNUMERIC
    )


def initialize_csv(output_csv=output_path):
    """Initialize the CSV file with a header row."""
    with open(output_csv, "w") as f:
        f.write("restaurant_id,menu\n")


def get_dishes(id_list, output_csv=output_path, batch_size=200):
    """Fetch menu data for a list of restaurant IDs in batches."""
    initialize_csv(output_csv)
    results = []
    session = init_session()
    total_len = len(id_list)

    with concurrent.futures.ThreadPoolExecutor(max_workers=50) as executor:
        for start in tqdm(range(0, total_len, batch_size), desc="Processing batches"):
            batch_ids = id_list[start : start + batch_size]
            futures = {
                executor.submit(
                    get_dish_details, session, int(row["id"]), int(row["restaurant_id"])
                ): (int(row["id"]), int(row["restaurant_id"]))
                for _, row in batch_ids.iterrows()
            }

            for future in concurrent.futures.as_completed(futures):
                try:
                    result = future.result()
                    if result:
                        results.append(result)
                except Exception:
                    pass

            if results:
                write_to_csv(output_csv, results)
                results = []


if __name__ == "__main__":
    get_dishes(id_list)
