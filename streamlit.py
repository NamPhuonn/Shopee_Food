...
# selected_ngram = st.slider("Chọn số lượng từ", min_value=1, max_value=6, value=2)
...
#     ax.set_title("Top 10 Từ khóa phổ biến trong thực đơn")
#     ax.set_xlabel("Điểm TF-IDF")
...
#     filtered_menus = []
#     for keyword in top_keywords["keyword"]:
#         # Lọc các món ăn chứa từ khóa
#         keyword_menus = filtered_df[filtered_df["food_name"].str.contains(keyword, case=False, na=False)].copy()
#         keyword_menus["keyword"] = keyword  # Gắn từ khóa vào DataFrame
#         filtered_menus.append(keyword_menus)
...
#             width=0.6
#         )
#         ax.set_title("Phân khúc giá của các từ khóa phổ biến")
...
#     # Cập nhật phần input cho người dùng
#     selected_keywords = st.multiselect("Chọn từ khóa", top_keywords["keyword"].tolist())
#     if selected_keywords:
#         filtered_menus = filtered_df[filtered_df["food_name"].str.contains('|'.join(selected_keywords), case=False, na=False)]
#         max_price = filtered_menus["food_price"].max()
...
#     else:
#         min_price = 0
#         max_price = 500000  # Mặc định giá trị nếu không có từ khóa nào

#     # Cập nhật slider với giá trị tối đa và giá trị mặc định
#     min_price, max_price_slider = st.slider(
#         "Chọn phạm vi giá", 
#         min_value=min_price, 
#         max_value=max_price, 
...
#         step=10000
#     )
#     # Lọc món ăn theo từ khóa và phạm vi giá
#     if selected_keywords and not filtered_df.empty:
#         # Lọc món ăn theo các từ khóa đã chọn
#         filtered_menus = filtered_df[filtered_df["food_name"].str.contains('|'.join(selected_keywords), case=False, na=False)]
        
#         # Lọc thêm theo phạm vi giá
#         filtered_menus = filtered_menus[(filtered_menus["food_price"] >= min_price) & (filtered_menus["food_price"] <= max_price_slider )]
